...
export default function Checkout() {
  ...
  const plan     = (sp.get("plan") as Plan) ?? "medium";
  const price    = Number(sp.get("price") ?? 0);
  const first    = sp.get("firstName") ?? "";
  const last     = sp.get("lastName") ?? "";
  ...
  const [nameOnCard, setNameOnCard] = useState(`${first} ${last}`.trim());
  ...
  async function handlePay() {
    ...
    try {
      ...
      const res = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ userId, plan, price, cardNumber, expiry, cvc }),
      });
  ...
  /* UI forkortet – behold gjerne din tidligere stil */
  return (
